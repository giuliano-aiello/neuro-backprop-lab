\begin{section}{Comparisons}
    ...
    The learning rate $\eta$ is set as $1\mathrm{e}{-3}$, $\eta^{-}$ and $\eta^{+}$ are $0.5$ and $1.2$, $\Delta_{\min}$ and $\Delta_{\max}$ are $1\mathrm{e}{-6}$ and $50$, the procedure was performed over $50$ epochs. A coherent configuration file varying on a specific~\glsxtrshort{rprop} version is:\\
    \begin{verbatim}
